...
\#Key Values from Partial Likelihood Ratio Test
...
predefined $\alpha = 0.05$ we get $G = 57.0778 \sim \chi^2_4$ with a
`p < 0.001` (R rounds to 0). As p \< 0.05, we reject the null hypothesis
that age, menopausal status, tumor size, and number of involved nodes
all together have no effect on the hazard of recurrence
($H_0 : \beta_age = \beta_menopausal = \beta_size = \beta_nodes = 0$).
...
association between hormone therapy and recurrence does not change as
the number of nodes changes ($H_0 : \beta_{nodes*hormone} = 0$). Further
more, these results suggest that Model 3 provides a significantly better
fit than Model 2: the addition of the interaction term improves model
fit.
...
##### Plot calculate and plot the survival function for the specified patient

The survival function $S(t\mid X)$ is the survival probability over time
for one specific patient or $P(T>t \mid X))$

##### Survival Curve of Specified Patient

<img src="HW_5_files/figure-gfm/unnamed-chunk-18-1.png" width="90%" style="display: block; margin: auto;" />

##### Probabilty of patient survival beyond 3 years (1095 days)

$$P(T>1095\mid X) = S(1095 \mid X)$$

##### Survival Statistics at 3 years for Specified Patient

| Time | Survival Probability | Std. Error | Lower 95% CI | Upper 95% CI |
|-----:|---------------------:|-----------:|-------------:|-------------:|
| 1095 |                0.705 |      0.034 |        0.642 |        0.774 |

Predicted 3-Year Survival Probability (Model 3)

Interpreting output: $S(1095 \mid X) = 0.705$: The estimated probability
that this patient survives more than 1095 days (3 years) is 70.5%.

`lower 95% CI: 0.642 Upper 95% CI 0.774:` We are 95% confident that the
true probability this patient survives more than 3 years lies between
64.2% and 77.4%.

`n.risk = 333:` The number of patients that were still in study (under
observation) by t=1095.

`n.event = 224:` The total number of events that had occured by the time
we had reached t=224.
